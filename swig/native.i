...
%native(AddHeroSkinTranslationEntry) int AddHeroSkinTranslationEntry(lua_State *lua);
...
static int AddHeroSkinTranslationEntry(lua_State *lua) {
    const char *key = luaL_checkstring(lua, 1);
    const char *value = luaL_checkstring(lua, 2);

    Sanguosha->addHeroSkinTranslationEntry(key, value);
