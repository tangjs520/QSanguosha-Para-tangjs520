    ...
    const QString &card_str, const QString &arg, const QString &arg2)
{
    ...
    QString finalLogText = append(log);
    if (type == "#Guhuo" || type == "#GuhuoNoTarget") {
        int pos = finalLogText.indexOf(tr("specify"));
        if (pos != -1) {
            finalLogText.insert(pos, "<br/>");
        }
        m_specialSkillLogText = finalLogText;
    }
}
...
{
    QString logText = QString("<p style=\"margin:3px 2px; line-height:120%;\">%1</p>").arg(text);
    QTextEdit::append(logText);
    return logText;
}
