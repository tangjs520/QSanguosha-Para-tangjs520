...
QWidget *ServerDialog::createAdvancedTab() {
    ...
    //Config.ServerPort则会是随机值，其他客户端无法探测到服务器”的问题
...
QWidget *ServerDialog::createMiscTab() {
    ...
    luck_card_spinbox->setRange(0, 5);
...
void ServerDialog::onDetectButtonClicked() {
    ...
    //之前使用的网站地址"http://iframe.ip138.com/ic.asp"有时会连接不上
...
void ServerDialog::replyFinished(QNetworkReply *reply)
{
    ...
    //从网站回复的信息中解析出外网IP地址，该网站的回复一般包括如下形式：
    //您的IP地址是：[XXX.XXX.XXX.XXX] 来自：XXX省XXX市 XXX
    ...
    , current(NULL), m_requestDeleteSelf(false)
{
    ...
    //单机模式时，不需要在此调用ServerInfoStruct::parse函数，
    //因为在Client::setup(const QString &setup_str)函数中会再调用一次
