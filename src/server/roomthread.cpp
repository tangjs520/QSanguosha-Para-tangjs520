...
bool JudgeStruct::isGood() const{
    ...
    bool ret = (_m_result == TRIAL_RESULT_GOOD);
    if (ret) {
        Sanguosha->playSystemAudioEffect("judge-good");
    }
    return ret;
}
...
RoomThread::RoomThread(Room *room)
    : room(room)
    , m_requestExit(false), game_rule(NULL), basara_rule(NULL)
{
}
...
void RoomThread::run() {
    ...
    if (Config.EnableBasara) {
        basara_rule = new BasaraMode(this);
        addTriggerSkill(basara_rule);
    }
    ...
    try {
        ...
        room->broadcastPhaseEffect(GameStart);
...
bool RoomThread::trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *target, QVariant &data) {
    m_mutexForRequestExit.lock();
    if (m_requestExit) {
        m_mutexForRequestExit.unlock();
        throw GameFinished;
    }
    m_mutexForRequestExit.unlock();
...
void RoomThread::addTriggerSkill(const TriggerSkill *skill) {
    ...
    if (skill->isVisible()) {
        QList<const Skill *> relatedSkills = Sanguosha->getRelatedSkills(skill->objectName());
...
void RoomThread::delay(long msecs) {
    if (msecs == -1) msecs = (Config.AIDelay / 1.5);
    Q_ASSERT(msecs >= 0);
