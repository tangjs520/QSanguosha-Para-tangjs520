...
void Settings::init() {
    ...
    if (!qApp->arguments().contains("-server")) {
        QString font_file_name = QFile::exists("font/font.ttf") ? "font/font.ttf" : "font/simli.ttf";
        QString font_path = value("DefaultFontPath", font_file_name).toString();
    ...
    Audio::setBackgroundMusicVolume(BGMVolume);
    Audio::setEffectVolume(EffectVolume);

    BackgroundImage = value("BackgroundImage", "image/system/backdrop/default.jpg").toString();
    GameBackgroundImage = value("GameBackgroundImage", "image/system/backdrop/tableBg.jpg").toString();
    ...
    QStringList limitedGeneralNames = Sanguosha->getLimitedGeneralNames();
    foreach (const QString &general, limitedGeneralNames) {
    ...
    if (banlist.isEmpty()) {
        foreach (const QString &ban_general, bossmode_ban)
            banlist << ban_general;
    ...
    foreach (const QString &key, jiange_defense_kingdoms.keys()) {
    ...
    foreach (const QString &key, jiange_defense_machine.keys()) {
    ...
    foreach (const QString &key, jiange_defense_soul.keys()) {
