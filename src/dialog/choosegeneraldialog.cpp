    ...
    : QDialog(parent)
{
    ...
    int columns = tooManyGenerals ? G_COMMON_LAYOUT.m_chooseGeneralBoxSwitchIconEachRowForTooManyGenerals :
    ...
    if (!generals.isEmpty()) {
        default_name = generals.first()->objectName();
    ...
    if (!view_only) {
        ...
        connect(mapper, SIGNAL(mapped(const QString &)),
            ClientInstance, SLOT(onPlayerChooseGeneral(const QString &)));
    }
    ...
    //禁止改变对话框的大小
    setFixedSize(dialog_layout->sizeHint());
}

void ChooseGeneralDialog::freeChoose() {
    FreeChooseDialog dialog(this);
    ...
    : QDialog(parent), type(type)
{
    ...
    foreach (const QString &kingdom, kingdoms) {
        ...
        if (!generals.isEmpty()) {
            ...
            scrollArea->setMinimumSize(652, 376);
            scrollArea->setWidgetResizable(true);

            tab_widget->addTab(scrollArea,
