...
public:
    explicit CustomAssignDialog(QWidget *parent);

    QString setListText(const QString &name, const QString &role, int index = -1);
...
private slots:
    ...
    void updatePlayerInfo(const QString &name);
    void updatePlayerHpInfo(const QString &name);
    void updateAllRoles(bool toggled = false);
    void updatePlayerExSkills(const QStringList &update_skills);
...
public slots:
    void getChosenGeneral(const QString &general_name);
...
public:
    explicit CardAssignDialog(QWidget *parent = 0, const QString &card_type = QString(),
        const QString &class_name = QString(), const QList<int> &excluded = QList<int>());
...
public:
    SkillAssignDialog(QDialog *parent, const QString &player_name, const QStringList &player_skills);
...
private slots:
    ...
    void getSkillFromGeneral(const QString &general_name);

    virtual void accept();

signals:
    void skill_update(const QStringList &);
};
