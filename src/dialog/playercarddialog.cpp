...
PlayerCardDialog::PlayerCardDialog(const ClientPlayer *player, const QString &flags,
                                   bool handcard_visible, Card::HandlingMethod method, const QList<int> &disabledIds)
    : player(player), handcard_visible(handcard_visible), method(method), disabled_ids(disabledIds)
{
...
QWidget *PlayerCardDialog::createAvatar() {
    ...
    if (player->getGeneral2()) {
        ...
        avatar2->setPixmap(QPixmap(G_ROOM_SKIN.getGeneralPixmap(player->getGeneral2Name(),
