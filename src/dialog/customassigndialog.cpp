...
QString CustomAssignDialog::setListText(const QString &name, const QString &role, int index) {
...
void CustomAssignDialog::updatePlayerInfo(const QString &name) {
    equip_list->clear();
    hand_list->clear();
    judge_list->clear();
...
void CustomAssignDialog::updatePlayerHpInfo(const QString &name) {
...
void CustomAssignDialog::doGeneralAssign() {
    ...
    connect(&dialog, SIGNAL(accepted()), this, SLOT(accept()));
    connect(&dialog, SIGNAL(general_chosen(const QString &)),
        this, SLOT(getChosenGeneral(const QString &)));
    dialog.exec();
}
...
void CustomAssignDialog::getChosenGeneral(const QString &name) {
    if (choose_general2) {
...
void CustomAssignDialog::doSkillSelect() {
    ...
    connect(&dialog, SIGNAL(skill_update(const QStringList &)),
        this, SLOT(updatePlayerExSkills(const QStringList &)));
    dialog.exec();
}

void CustomAssignDialog::updatePlayerExSkills(const QStringList &update_skills) {
    QString name = list->currentItem()->data(Qt::UserRole).toString();
...
GeneralAssignDialog::GeneralAssignDialog(QWidget *parent, bool can_ban): QDialog(parent) {
    ...
    QList<const General *> all_generals = Sanguosha->getAllGenerals();
...
CardAssignDialog::CardAssignDialog(QWidget *parent, const QString &card_type,
    const QString &class_name, const QList<int> &excluded)
...
SkillAssignDialog::SkillAssignDialog(QDialog *parent, const QString &player_name, const QStringList &player_skills)
...
void SkillAssignDialog::selectSkill() {
    GeneralAssignDialog dialog(this);
    connect(&dialog, SIGNAL(general_chosen(const QString &)),
        this, SLOT(getSkillFromGeneral(const QString &)));
    dialog.exec();
}
...
void SkillAssignDialog::getSkillFromGeneral(const QString &general_name) {
